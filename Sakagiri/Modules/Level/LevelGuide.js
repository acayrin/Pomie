 ...
 *      - additional args:
 * 
 *          -a/--auto: automatically calculate the exp bonus per level
...
module.exports.getLevelGuide = async (args) => {
    ...
    // get data
    const total = await Utils.resolve(getMobList(level, _bonus === 'auto' ? getExpBonus(level) : _bonus))
    ...
    if (level && _level) {
        ...
        for (let _lvl = Number(level); _lvl < _alevel; _lvl++) {
            ...
            const _mlist       = await Utils.resolve(getMobList(_lvl,     _cbonus))
            const _mlist_2     = await Utils.resolve(getMobList(_lvl + 1, _cbonus))
