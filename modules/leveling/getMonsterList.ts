...
> {
	...
	return new Promise(async (resolve) => {
		...
		for (let index = monsterList.length; --index >= 0; ) {
			// variable
			const monsterEntry = monsterList[index] as ToramMonster;

			// ignore event mobs
			if (
				(mod.getConfig().ignored_leveling?.id as string[]).includes(
					monsterEntry.id,
				) ||
				(mod.getConfig().ignored_leveling?.map as string[]).includes(
					monsterEntry.map,
				) ||
				(mod.getConfig().ignored_leveling?.name as string[]).includes(
					monsterEntry.name,
				)
			)
				continue;
			...
			const monsterLevel = Number(monsterEntry.level)
				? Number(monsterEntry.level)
				: 1;
			const monsterExp = Number(monsterEntry.exp)
				? Number(monsterEntry.exp)
				: 1;
